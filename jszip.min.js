    ...
    event.waitUntil(
        caches.keys().then(cacheNames => Promise.all(
            cacheNames.map(cacheName => {
                // CORRIGÉ : On conserve bien TOUS les caches nécessaires
...
self.addEventListener('fetch', event => {
    ...
    if (requestUrl.hostname.includes('tile.openstreetmap.org')) {
        event.respondWith(
            // 1. Chercher dans le cache de tuiles téléchargées
            caches.match(event.request, { cacheName: TILE_CACHE_NAME_OFFLINE }).then(response => {
                return response || caches.open(TILE_CACHE_NAME_DYNAMIC).then(cache => {
                    // 2. Sinon, chercher dans le cache de tuiles de navigation
                    return cache.match(event.request).then(response => {
                        // 3. Sinon, aller sur le réseau (et mettre en cache dans le cache dynamique)
