...
const SearchToggleControl = L.Control.extend({ options: { position: 'topleft' }, onAdd: function (map) { const mainContainer = L.DomUtil.create('div', 'leaflet-control'); const topBar = L.DomUtil.create('div', 'leaflet-bar search-toggle-container', mainContainer); this.toggleButton = L.DomUtil.create('a', 'search-toggle-button', topBar); this.toggleButton.innerHTML = '🔍'; this.toggleButton.href = '#'; this.communeDisplay = L.DomUtil.create('div', 'commune-display-control', topBar); const versionDisplay = L.DomUtil.create('div', 'version-display', mainContainer); versionDisplay.innerText = 'v1.3'; L.DomEvent.disableClickPropagation(mainContainer); L.DomEvent.on(this.toggleButton, 'click', L.DomEvent.stop); L.DomEvent.on(this.toggleButton, 'click', () => { const uiOverlay = document.getElementById('ui-overlay'); const isHidden = uiOverlay.style.display === 'none'; if (isHidden) { uiOverlay.style.display = 'block'; this.communeDisplay.style.display = 'none'; } else { uiOverlay.style.display = 'none'; if (this.communeDisplay.textContent) { this.communeDisplay.style.display = 'block'; } } }); return mainContainer; }, setName: function(name) { this.communeDisplay.textContent = name; } });
