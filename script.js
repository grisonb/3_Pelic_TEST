...
const calculateBearing = (lat1, lon1, lat2, lon2) => { const lat1Rad = toRad(lat1); const lon1Rad = toRad(lon1); const lat2Rad = toRad(lat2); const lon2Rad = toRad(lon2); const dLon = lon2Rad - lon1Rad; const y = Math.sin(dLon) * Math.cos(lat2Rad); const x = Math.cos(lat1Rad) * Math.sin(lat2Rad) - Math.sin(lat1Rad) * Math.cos(lat2Rad) * Math.cos(dLon); let bearingRad = Math.atan2(y, x); let bearingDeg = toDeg(bearingRad); return (bearingDeg + 360) % 360; };
const convertToDMM = (deg, type) => { if (deg === null || isNaN(deg)) return 'N/A'; const absDeg = Math.abs(deg); const degrees = Math.floor(absDeg); const minutesTotal = (absDeg - degrees) * 60; const minutesFormatted = minutesTotal.toFixed(2).padStart(5, '0'); let direction = type === 'lat' ? (deg >= 0 ? 'N' : 'S') : (deg >= 0 ? 'E' : 'W'); return `${degrees}° ${minutesFormatted}' ${direction}`; };
...
window.toggleAirport = (oaci) => { if (disabledAirports.has(oaci)) disabledAirports.delete(oaci); else { disabledAirports.add(oaci); waterAirports.delete(oaci); } saveState(); refreshUI(); };
window.toggleWater = (oaci) => { if (waterAirports.has(oaci)) waterAirports.delete(oaci); else { waterAirports.add(oaci); disabledAirports.delete(oaci); } saveState(); refreshUI(); };
const SearchToggleControl = L.Control.extend({ options: { position: 'topleft' }, onAdd: function (map) { const mainContainer = L.DomUtil.create('div', 'leaflet-control'); const topBar = L.DomUtil.create('div', 'leaflet-bar search-toggle-container', mainContainer); this.toggleButton = L.DomUtil.create('a', 'search-toggle-button', topBar); this.toggleButton.innerHTML = '🔍'; this.toggleButton.href = '#'; this.communeDisplay = L.DomUtil.create('div', 'commune-display-control', topBar); const versionDisplay = L.DomUtil.create('div', 'version-display', mainContainer); versionDisplay.innerText = 'v1.5'; L.DomEvent.disableClickPropagation(mainContainer); L.DomEvent.on(this.toggleButton, 'click', L.DomEvent.stop); L.DomEvent.on(this.toggleButton, 'click', () => { const uiOverlay = document.getElementById('ui-overlay'); const isHidden = uiOverlay.style.display === 'none'; if (isHidden) { uiOverlay.style.display = 'block'; this.communeDisplay.style.display = 'none'; } else { uiOverlay.style.display = 'none'; if (this.communeDisplay.textContent) { this.communeDisplay.style.display = 'block'; } } }); return mainContainer; }, setName: function(name) { this.communeDisplay.textContent = name; } });
function soundex(s) { if (!s) return ''; const a = s.toLowerCase().split(''); const f = a.shift(); if (!f) return ''; let r = ''; const codes = { a: '', e: '', i: '', o: '', u: '', b: 1, f: 1, p: 1, v: 1, c: 2, g: 2, j: 2, k: 2, q: 2, s: 2, x: 2, z: 2, d: 3, t: 3, l: 4, m: 5, n: 5, r: 6 }; r = f + a.map(v => codes[v]).filter((v, i, a) => (i === 0) ? v !== codes[f] : v !== a[i - 1]).join(''); return (r + '000').slice(0, 4).toUpperCase(); }
